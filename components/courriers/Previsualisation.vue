<template>
   <div class="container-pdf">
      <div>
         <client-only>
            <vue-html2pdf :show-layout="false" :enable-download="true" :preview-modal="false"
               :paginateElementsByHeight="1200" :manual-pagination="true" :html-to-pdf-options="htmlToPdfOptions"
               @progress="onProgress($event)" @hasStartedGeneration="hasStartedGeneration()"
               @beforeDownload="beforeDownload($event)" @hasGenerated="hasGenerated($event)"
                        ...
                        <form>
                           <div class="row border-grey">
                              <div class="form-group col-8">
                                 <p><span>Prénom : </span>{{ this.detailutilisateur &&
                                    this.detailutilisateur.requerant.prenom }}</p>
                                 <p><span>Nom : </span>{{ this.detailutilisateur && this.detailutilisateur.requerant.nom }}
                                 </p>
                                 <p><span>Email : </span>{{ this.detailutilisateur &&
                                    this.detailutilisateur.requerant.email }}</p>
                                 <p><span>Téléphone : </span>{{ this.detailutilisateur &&
                                    this.detailutilisateur.requerant.telephone }}</p>
                                 <p><span>Adresse : </span>{{ this.detailutilisateur &&
                                    this.detailutilisateur.requerant.adresse }}</p>
                              </div>
